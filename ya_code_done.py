...
class VK:
    ...
    def get_photos(self):
        response = requests.get(f'{self.url_vk}photos.getAll', params = self.get_common_params(user_id))
        return response.json()
    
    def load_foto(self, data):
        for vk_photo_json in data['response']['items']:
            file_url = vk_photo_json['sizes'][-1]['url']
            sizes_file = vk_photo_json['sizes'][-1]['type']
            name_likes = vk_photo_json['likes']['count']
            name_date = dt.fromtimestamp(vk_photo_json['date']).strftime('%Y_%m_%d_%H_%M_%S')
            self.name_date_list.append(dt.fromtimestamp(vk_photo_json['date']).strftime('%Y_%m_%d_%H_%M_%S'))
            self.name_likes_list.append(vk_photo_json['likes']['count'])
            name_id = vk_photo_json['id']
            self.name_id_list.append(vk_photo_json['id'])    
            time.sleep(0.1)
...
class Yandex:
    ...
    def load_photo_to_ya_dick(self, headers, vk_name):
        self.vk_name = vk_name
        for name_photo in self.vk_name:
            res_likes = requests.get(f'{self.url_yad}/upload?path=/test/{name_photo}.jpg', headers=headers).json()
            res_likes_json = requests.get(f'{self.url_yad}/upload?path=/test/{name_photo}.json', headers=headers).json()
            with open(f'images/{name_photo}.jpg', 'rb') as f:
                requests.put(res_likes['href'], files={'file':f})
            with open(f'images/{name_photo}.json', 'rb') as f:
                requests.put(res_likes_json['href'], files={'file':f})

token = Get_token()
access_token = token.get_token_vk()
...
vk.load_foto(data)
ya.load_photo_to_ya_dick(headers, vk_name)